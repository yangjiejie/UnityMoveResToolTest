...
public static class EasyUseEditorTool  // 简称euetool 
{
    ...
    static private void GotoCSharpCodeLine()
    {
        ...
        if (listPath.Count > 0)
        {
            ...
            if(tmpCsName.StartsWith(GenerateScriptWindow.preFix))
            {
                targetGenCsFile = targetInterfaceCsFile;
                targetInterfaceCsFile = targetGenCsFile.Replace(GenerateScriptWindow.preFix, "");
            }
            else
            {
                var folderName = Path.GetDirectoryName(targetInterfaceCsFile);
                targetGenCsFile = Path.Combine(folderName, GenerateScriptWindow.preFix + tmpCsName);
            }

...
using Assets.HotFix.CComponent.UtilFolder;
using System.Linq;
using RTLTMPro;
...
public class RightBtnMenu
{
    ...
    [MenuItem("GameObject/右键菜单/批量处理字体断开", priority = 1)]
    static void BatDelFontBlod()
    {
        var allPrefab =  AssetDatabase.FindAssets("t:prefab", new string[]
        {
            "Assets/Art/gameBC/prefab",
            "Assets/Art/gameFruit/prefab",
            "Assets/Art/hall/prefab"

        }).Select((xx) =>AssetDatabase.GUIDToAssetPath(xx)).ToList();
        foreach (var prefab in allPrefab)
        {
            var resGo = AssetDatabase.LoadAssetAtPath<GameObject>(prefab);
            bool needRepace = false;
            var coms = resGo.GetComponentsInChildren<RTLTextMeshPro>();
            foreach (var com in coms)
            {
                if (com.fontStyle == TMPro.FontStyles.Normal)
                {
                    continue;
                }
                com.fontStyle = TMPro.FontStyles.Normal;
    ...
    [MenuItem("GameObject/右键菜单/执行ui布局/1", priority = 1)]
    static void ExxUILayout1()
    {
        var go = Selection.activeGameObject;
        var coms = go.GetComponentsInChildren<ImageBgSizeChange>();
        foreach(var item in coms)
        {
            item.ApplyRecord1();
        }
    }
    [MenuItem("GameObject/右键菜单/执行ui布局/2", priority = 1)]
    static void ExxUILayout2()
    {
        var go = Selection.activeGameObject;
        var coms = go.GetComponentsInChildren<ImageBgSizeChange>();
        foreach (var item in coms)
